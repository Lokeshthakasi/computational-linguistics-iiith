...
function lang1() {
  b =
    correct_hindi_sentence[
      Math.floor(Math.random() * correct_hindi_sentence.length)
    ];
  b1 = b.split(" ");
  a =
    correct_english_sentence[
      Math.floor(Math.random() * correct_english_sentence.length)
    ];
  a1 = a.split(" ");
  ...
  if (document.getElementById("english").selected) {
    ...
    for (i = 0; i < a1.length; i++) {
      ...
      button.addEventListener("click", function () {
        ...
        var x = String(document.getElementById("example5").innerHTML).replace(
          /\s+$/,
          ""
        );
        x1 = x.split(" ");
...
function analyse() {
  var m = String(document.getElementById("example5").innerHTML).replace(
    /\s+$/,
    ""
  );
  n = 0;
